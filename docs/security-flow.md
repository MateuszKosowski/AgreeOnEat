# Przepływ uwierzytelniania i autoryzacji w AgreeOnEat

## Obecne zasady i granice zaufania

1. Keycloak jako jedyny uwierzytelnia użytkowników, przechowuje hashe haseł i wystawia tokeny.
2. API Gateway jest jedynym publicznym wejściem do biznesowego API.
3. Gateway i każdy mikroserwis biznesowy są osobnymi OAuth2 Resource Serverami i niezależnie weryfikują otrzymany JWT.
4. Gateway wymaga poprawnego JWT dla `/api/**`, a mikroserwisy dodatkowo wymagają `ROLE_USER`.
5. Publiczne są wyłącznie wymagane endpointy health/info. Pozostałe, niewymienione ścieżki są blokowane.
6. Stabilnym identyfikatorem użytkownika jest claim `sub`, a nie zmienny adres e-mail.
7. JWT jest podpisany, ale nie jest zaszyfrowany. Tokeny, hasła i sekrety nie mogą trafiać do repozytorium, logów ani odpowiedzi biznesowych.
8. HTTP jest używany wyłącznie w lokalnym środowisku deweloperskim. Poza nim ruch do Keycloak i API Gateway wymaga HTTPS.
...
Scope’y operacyjne API mogą mieć nazwy przypominające permisje, na przykład:
...
Za bezpieczne losowanie, tymczasowe przechowywanie i sprawdzanie tych wartości odpowiada biblioteka OIDC używana przez frontend.
...
Po otwarciu przeglądarki aplikacja oczekuje na późniejszy powrót przez skonfigurowany callback. Poza lokalnym środowiskiem deweloperskim endpoint Keycloak musi być dostępny przez HTTPS.
...
| `code_challenge` | Wyliczony w kroku 3 z `code_verifier`. | Wiąże authorization code utworzony w tej próbie logowania z jednorazowym sekretem pozostającym na urządzeniu. |
...
Formularz działa na stronie Keycloak otwartej w systemowej przeglądarce. Wygląd strony można dostosować za pomocą motywu Keycloak.
...
Żądanie nie przechodzi przez frontend React Native, API Gateway ani żaden mikroserwis. Dzięki temu tylko Keycloak ma dostęp do hasła użytkownika. Lokalnie komunikacja używa HTTP wyłącznie na potrzeby developmentu; poza tym środowiskiem formularz i dane logowania muszą być przesyłane przez HTTPS.
...
Słowo `Bearer` oznacza, że dostęp otrzymuje posiadacz poprawnego tokenu. Z tego powodu token nie może trafiać do adresu URL, logów ani komunikacji bez szyfrowania. Poza lokalnym środowiskiem deweloperskim żądania do Gateway muszą korzystać z HTTPS.

API Gateway działający lokalnie na porcie `8080` jest punktem wejścia do biznesowej części backendu. Frontend nie wybiera konkretnej instancji mikroserwisu i nie wysyła do niej żądania bezpośrednio. Użyta ścieżka, na przykład `/api/users/**`, pozwala Gateway rozpoznać usługę docelową.
...
Natywna aplikacja mobilna i Postman nie podlegają mechanizmowi CORS przeglądarki. Gateway dopuszcza obecnie klienta przeglądarkowego z originem `http://localhost:3000`, metody `GET`, `POST`, `PUT`, `PATCH`, `DELETE`, `OPTIONS` oraz skonfigurowany zestaw nagłówków. Przeglądarka może przed właściwym żądaniem automatycznie wysłać zapytanie `OPTIONS`.
...
Technicznie JWT ma postać:

```text
BASE64URL(header).BASE64URL(payload).BASE64URL(signature)
```

Base64URL jest kodowaniem, a nie szyfrowaniem, dlatego każdy posiadacz tokenu może odczytać jego header i payload. Integralność zapewnia podpis JWS, natomiast poufność tokenu podczas przesyłania zapewnia HTTPS.

`RS256` oznacza RSASSA-PKCS1-v1_5 z funkcją skrótu SHA-256. Keycloak tworzy podpis prywatnym kluczem RSA, a Gateway weryfikuje go odpowiadającym kluczem publicznym. AgreeOnEat nie implementuje własnej kryptografii — konfiguruje gotowe komponenty Spring Security i ogranicza akceptowany algorytm oraz claimy.

Dla żądania z Bearer tokenem wewnętrzny łańcuch Spring Security wygląda w uproszczeniu tak:

```text
BearerTokenAuthenticationFilter
    → odczytuje token z nagłówka Authorization
JwtAuthenticationProvider
    → przekazuje JWT do dekodera
NimbusJwtDecoder
    → wybiera JWK, sprawdza podpis i uruchamia walidatory claimów
SecurityFilterChain
    → podejmuje decyzję o dostępie do ścieżki
```

Kod konfigurujący ten proces znajduje się w [`GatewaySecurityConfig`](../api-gateway/src/main/java/pl/agreeoneat/apigateway/config/security/GatewaySecurityConfig.java). Polityka cache JWKS nie jest nadpisana przez AgreeOnEat — zarządza nią Spring Security.
...
| `iss` | `http://localhost:8081/realms/agreeoneat` w środowisku lokalnym | Potwierdza, że token został wystawiony przez realm wskazany w konfiguracji danego środowiska. |
...
| `nbf` | Jeżeli występuje, bieżący czas nie może być wcześniejszy od tej wartości. | Powoduje odrzucenie tokenu, który jeszcze nie zaczął obowiązywać. |
...
`issuer-uri` jest oczekiwaną tożsamością wystawcy i musi dokładnie odpowiadać claimowi `iss`. `jwk-set-uri` jest natomiast technicznym adresem pobierania kluczy, dlatego wewnątrz Dockera może wskazywać `keycloak:8080`, mimo że `iss` nadal zawiera adres `localhost:8081`. Jawne `jwk-set-uri` pozwala pobrać klucze bez wykonywania OIDC Discovery podczas uruchamiania usługi.
...
Jeżeli token jest poprawny, Spring Security tworzy uwierzytelnienie użytkownika dla bieżącego żądania. Gateway działa bezstanowo (`STATELESS`), więc nie tworzy własnej sesji logowania i wykonuje tę weryfikację przy każdym następnym żądaniu z access tokenem. Poprawny token nie oznacza jeszcze automatycznej zgody na każdą ścieżkę. Za konfigurację CORS odpowiada [`GatewayCorsConfig`](../api-gateway/src/main/java/pl/agreeoneat/apigateway/config/security/GatewayCorsConfig.java), a za odpowiedzi `401` i `403` — [`RestSecurityErrorHandler`](../api-gateway/src/main/java/pl/agreeoneat/apigateway/config/security/RestSecurityErrorHandler.java).
...
Po wybraniu instancji Gateway działa jak reverse proxy: tworzy połączenie z wewnętrznym adresem wybranego mikroserwisu — w tym przykładzie `user-service` — i przekazuje do niego żądanie użytkownika.
...
Ponowna weryfikacja w mikroserwisie jest celowa. Gateway chroni wejście do systemu, ale mikroserwis nie zakłada, że każde żądanie z sieci wewnętrznej na pewno przeszło przez Gateway. Żądanie może pochodzić z innego kontenera, błędnie skonfigurowanej trasy, podatności SSRF albo przejętego komponentu wewnętrznego. Mikroserwis stanowi więc drugą granicę bezpieczeństwa i sam sprawdza poświadczenie, na podstawie którego wykonuje logikę biznesową.
...
### 23. Pobranie kluczy publicznych Keycloak przez mikroserwis

Po odebraniu żądania Spring Security w mikroserwisie odczytuje z nagłówka access tokenu identyfikator `kid`. Podobnie jak wcześniej Gateway, mikroserwis potrzebuje odpowiadającego mu klucza publicznego, zanim będzie mógł zaufać podpisowi JWT.

Gateway i mikroserwis są osobnymi procesami i mają osobne pamięci. Klucz pobrany wcześniej przez Gateway nie jest automatycznie przekazywany do mikroserwisu. Każda instancja usługi samodzielnie utrzymuje własny cache kluczy i wykonuje własną weryfikację kryptograficzną.

Jeżeli mikroserwis nie ma w pamięci klucza odpowiadającego `kid`, na przykład obsługuje pierwszy token po uruchomieniu albo Keycloak wykonał rotację kluczy, wysyła żądanie:
...
| Środowisko | Adres JWKS używany przez mikroserwis |
| --- | --- |
...
Mikroserwis nie wysyła do Keycloak access tokenu użytkownika z pytaniem, czy jest on poprawny. Nie korzysta tutaj z endpointu introspekcji. Pobiera jedynie publiczne klucze i samodzielnie sprawdza token lokalnie. Dzięki temu Keycloak nie musi uczestniczyć w obsłudze każdego żądania biznesowego.

Krok jest warunkowy. Jeśli pasujący klucz znajduje się już w cache danej instancji mikroserwisu, kroki 23 i 24 zostają pominięte. Przy wielu instancjach każda z nich posiada własny cache i w razie potrzeby pobiera JWKS niezależnie.

### 24. Zwrócenie zestawu JWKS do mikroserwisu

Keycloak odpowiada mikroserwisowi zestawem publicznych kluczy realmu `agreeoneat`. Jest to ten sam dokument JWKS, który wcześniej mógł pobrać API Gateway:
...
Wybrany zestaw kluczy zostaje zachowany w cache konkretnej instancji mikroserwisu. Nie jest współdzielony z Gateway ani z innymi instancjami.
...
Jeżeli po odświeżeniu JWKS nadal nie istnieje klucz odpowiadający `kid` tokenu albo zestawu nie uda się pobrać, mikroserwis nie może bezpiecznie potwierdzić podpisu. Token musi wtedy zostać odrzucony, a żądanie nie może dotrzeć do kontrolera.

### 25. Ponowna weryfikacja access tokenu przez mikroserwis

Filtry Spring Security przechwytują żądanie, zanim trafi ono do kontrolera. Mikroserwis traktuje otrzymany JWT jako niezaufany i samodzielnie wykonuje te same podstawowe kontrole kryptograficzne oraz czasowe co API Gateway.

Konfiguracja Resource Server w każdym mikroserwisie biznesowym zawiera:
...
Implementują ją odpowiednio [`UserServiceSecurityConfig`](../user-service/src/main/java/pl/agreeoneat/userservice/config/security/UserServiceSecurityConfig.java), [`HouseholdServiceSecurityConfig`](../household-service/src/main/java/pl/agreeoneat/householdservice/config/security/HouseholdServiceSecurityConfig.java), [`RecipeServiceSecurityConfig`](../recipe-service/src/main/java/pl/agreeoneat/recipeservice/config/security/RecipeServiceSecurityConfig.java) i [`MealPlanningServiceSecurityConfig`](../meal-planning-service/src/main/java/pl/agreeoneat/mealplanningservice/config/security/MealPlanningServiceSecurityConfig.java).
...
Weryfikacja podpisu odbywa się lokalnie przy użyciu klucza publicznego. Mikroserwis nie przesyła tokenu do Keycloak ani do Gateway i nie potrzebuje klucza prywatnego. Jeśli odpowiedni klucz jest już w cache, sprawdzenie nie wymaga żadnego dodatkowego połączenia sieciowego.
...
Jeżeli token przejdzie wszystkie kontrole, Spring Security tworzy dla bieżącego żądania obiekt uwierzytelnienia zawierający zweryfikowany JWT. Mikroserwis działa bezstanowo, więc nie tworzy własnej sesji i powtarza walidację przy każdym żądaniu.
...
Po zweryfikowaniu JWT mikroserwis odczytuje z niego informacje, które mają reprezentować uprawnienia użytkownika. W tokenie Keycloak są one zapisane jako zwykłe claimy, na przykład:
...
Role realmu znajdują się w zagnieżdżonym claimie `realm_access.roles`, którego standardowy converter Springa nie obsługuje. Dlatego każdy mikroserwis posiada dwie odpowiadające sobie klasy:
...
### 27. Autoryzacja ścieżki w mikroserwisie

Po utworzeniu uwierzytelnienia mikroserwis biznesowy podejmuje ostateczną decyzję, czy żądanie może dotrzeć do kontrolera i wykonać daną operację. Walidacja JWT z kroku 25 potwierdziła, kim jest wywołujący, natomiast autoryzacja odpowiada na pytanie, co wolno mu zrobić.

Każdy obecny mikroserwis biznesowy stosuje regułę:

```java
.requestMatchers("/api/**").hasRole("USER")
```

Ostateczny dostęp można więc przedstawić jako:
...
Poprawny JWT bez `ROLE_USER` kończy się odpowiedzią `403 Forbidden`, zanim żądanie dotrze do kontrolera.

#### Dlaczego `/api/users/me` nie potrzebuje dodatkowej permisji poza rolą `USER`?
...
Identyfikator bieżącego użytkownika pochodzi z `jwt.getSubject()`, czyli z podpisanego claima `sub`. Użytkownik nie może podstawić w żądaniu innego `userId` i poprosić o dane cudzej osoby. Endpoint zwraca informacje o podmiocie, którego token został właśnie zweryfikowany, dlatego wymaga bazowej reguły `hasRole("USER")` bez dodatkowej, bardziej szczegółowej permisji.

`/api/users/me` jest endpointem diagnostycznym i odczytuje dane bezpośrednio z tokenu. Nie pobiera profilu użytkownika z bazy i nie wykonuje operacji na zasobie należącym do innej osoby.
...
Mikroserwis nie umieszcza w odpowiedzi biznesowej:

- access tokenu ani refresh tokenu;
- authorization code;
- sekretów klientów;
- pełnych wyjątków bezpieczeństwa i stack trace;
- publicznych ani prywatnych danych kluczy.

Mikroserwis zwraca wyłącznie dane, do których dostęp został zaakceptowany w poprzednim kroku.
...
Dla klienta przeglądarkowego Gateway dodaje nagłówki CORS zgodnie z konfiguracją dozwolonych originów, obecnie `http://localhost:3000`. Natywna aplikacja mobilna nie korzysta z mechanizmu CORS.
...
- `403 Forbidden` oznacza, że tożsamość została rozpoznana, ale token nie zawiera wymaganej roli `USER`;
- pozostałe błędy są obsługiwane zgodnie z ich znaczeniem biznesowym lub technicznym.

Gateway nie zwraca frontendowi access tokenu przekazanego w żądaniu i nie wystawia nowego tokenu. Odpowiedź zawiera wyłącznie wynik wywołania API. Poza lokalnym środowiskiem deweloperskim komunikacja frontend ↔ Gateway musi odbywać się przez HTTPS.
